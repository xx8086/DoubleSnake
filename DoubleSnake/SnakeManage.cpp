...
void CSnakeManage::Init(int iWallWide, int iWallHigh, int iDescribeBeginX)
{
...
bool CSnakeManage::draw_wall(int iLeft, INT iTop, int iWide, int iHigh)
{
...
bool CSnakeManage::draw_describe(int iBeginX, int iBeginY, int irowheight)
{
	int iHight = iBeginY;
	int iRow = 0 ;
	setcolor(RED);
...
bool CSnakeManage::create_food(Food& food)
{
	srand(time(NULL));
...
bool CSnakeManage::init_snake_pos(Snake& red_snake, Snake& blue_snake)
{
	red_snake.life = 1;
...
bool CSnakeManage::snake_move(Snake& x_snake)
{
	if(0 == x_snake.life)
		return false;
...
bool CSnakeManage::life_check(Snake& x_snake)
{
	bool bLife = true;
	if(x_snake.life)
	{
...
bool CSnakeManage::draw_snake(Snake& x_snake, Food& food, COLORREF snake_color)
{
	if(0 == x_snake.life)
	{
		return false;
...
bool CSnakeManage::snake_directe(Snake& red_snake, Snake& blue_snake)
{
	//两条蛇都死了，返回false,结束游戏
...
void CSnakeManage::goodbay(void)
{
	cleardevice();
	draw_score();
	setcolor(RED);
...
void CSnakeManage::draw_score(void)
{
	wchar_t str[20] = {0};
	setfillstyle(YELLOW);
	bar(520, 10, 620, 50);
